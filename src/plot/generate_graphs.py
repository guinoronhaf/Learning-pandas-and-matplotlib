...
from aux import auxiliar_functions as a

PATH = '../graphs/'

def generate_graph_1(df):
    df_speciality = a.group_speciality_quantity(df)
    ...
    plt.savefig(PATH + "gráfico_porcentagem_cargo.png", bbox_inches='tight')

    plt.clf()

def generate_graph_2(df):
    df_experiense = a.group_percentage_years_specialist(df)
    ...
    plt.savefig(PATH + "gráfico_experiência_cargo.png")
...
def generate_graph_3(df: pd.DataFrame):
    ...
    plt.savefig(PATH + "grafico3.png")

    plt.clf()

def generate_graph_6(df: pd.DataFrame):
    df_to_plot = a.get_data_frame_app_rate(df)

    fig, ax = plt.subplots(figsize=(12, 8))

    dict_times_by_rate = a.map_times_by_rate(df_to_plot)
    ...
    plt.savefig(PATH + "ai_trust_rated_by_radiologies.png")
